...
public:
    ...
    KalmanFilter() {
        ...
        Q = {{pow(dt, 4)/4 * pow(sigma_a, 2), 0, pow(dt, 3)/2 * pow(sigma_a, 2), 0},
             {0, pow(dt, 4)/4 * pow(sigma_a, 2), 0, pow(dt, 3)/2 * pow(sigma_a, 2)},
             {pow(dt, 3)/2 * pow(sigma_a, 2), 0, pow(dt, 2) * pow(sigma_a, 2), 0},
             {0, pow(dt, 3)/2 * pow(sigma_a, 2), 0, pow(dt, 2) * pow(sigma_a, 2)}};
        
        // Initialize measurement covariance matrix
        R = {{pow(sigma_measurement, 2), 0},
             {0, pow(sigma_measurement, 2)}};
    }
    ...
    void update(vector<double> z) {
        ...
        vector<vector<double>> I = identity_matrix(4);
...
int main() {
    ...
    for (const auto& entry : predictedStates) {
        ...
        cout << "Target ID: " << targetID << ", Predicted State: [x=" << state[0] << ", y=" << state[1] << ", vx=" << state[2] << ", vy=" << state[3] << "]" << endl;
    }
